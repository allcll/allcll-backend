...
```
수강편람 PDF ─(wiki 정본 cohort/·mappings/)→ matrix ─(PR 리뷰 → apply CI)→ 구글시트 13탭 ─(sync)→ DB ─→ resolver/checker
                                              └ 2026-08 부터 자동. live 12탭이 apply 대상 (../conventions/loading-rules.md §7)
...
| 데이터 | 출처 | 적재/조회 코드 | 주의 |
|---|---|---|---|
| 이수구분별 요구학점 | 구글시트 `credit_criteria`·`double_credit_criteria` | `CreditCriterionRepository`, `DoubleCreditCriterionResolver` | wiki cohort/ 파생 (apply CI 반영). ALL_DEPT·페어 fallback |
...
| 동일/대체 과목 | 시트 `course_equivalences` (그룹 컬럼 = `same_course_code`, 구 `group_code`) | `CourseEquivalenceRepository` | `AcademicBasicPolicy`·`RequiredCourseResolver` 사용 |
...
4. 시트 데이터는 wiki 정본의 파생물 → **적재 규칙(../conventions/loading-rules.md) 준수 필요**. 시트 값이 틀렸으면 시트를 직접 고치지 말고 graduation-wiki `grad-sheet-change` 경로로 (§7 = apply 대상 탭 목록).

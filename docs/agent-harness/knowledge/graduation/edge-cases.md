...
- **조건**: 성적표 raw 카테고리 `[전기]`/`[복기]` (MAJOR_BASIC alias) 가 학번 < 2024 사용자의 행에 등장
...
- **이력**:
    ...
    - 20~23학번 복수 전공: 복수전공 전공기초는 `[복기]` 로 찍힘 — alias 부재 시 CATEGORY_TYPE_NOT_FOUND 로
      업로드 전체 실패 (2026-07 QA B8 20011143 발견, reports/qa/2026-07/triage.md B8 판정). MajorScope 는
      복필·복선과 동일하게 SECONDARY. pre-24 는 ACADEMIC_BASIC 보정 + SECONDARY → 기준 행 부재 시 카테고리
      미산입(TOTAL 만) — "AB 증발" 정책 결정 건(분류 A)과 동일 계열

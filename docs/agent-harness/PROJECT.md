...
환경에 맞게 덮어써야 함. 민감 정보가 포함될 수 있으므로 파일 내용을 그대로 출력하거나 외부로 공유하지 않는다.
...
- 신규·수정 테스트에서 **`Thread.sleep()` 금지** — 동시성 테스트엔 Awaitility 사용. 기존 테스트의 `Thread.sleep()`은 해당 테스트를
  건드리는 작업 범위에서만 교체한다.

자세한 테스트 작성 절차는 `docs/agent-harness/skills/testing/SKILL.md`.
...
- 작업 시작 전 `git status --short` 로 사용자 변경을 확인한다. 내가 만들지 않은 변경은 되돌리지 말고, 관련 파일이면 함께 읽고 맞춰서 수정한다.
...
- 공통 프로젝트 컨텍스트 원본은 `docs/agent-harness/PROJECT.md` 에 둔다.
- Claude Code 는 `CLAUDE.md` 에서 `docs/agent-harness/PROJECT.md` 를 import 하고, `.claude/` 에는 Claude 전용 wrapper 만 둔다.
- Codex 는 `AGENTS.md` 에서 `docs/agent-harness/PROJECT.md` 를 import 하고, `.agents/` 에는 Codex 전용 wrapper 만 둔다.
- skill 본문은 `docs/agent-harness/skills/*/SKILL.md` 한 곳에서 관리한다. `.claude/skills/*/SKILL.md` 와 `.agents/skills/*/SKILL.md` 는 discovery 용 frontmatter 와 공통 본문 참조만 가진다.
- command 본문은 `docs/agent-harness/commands/*.md` 한 곳에서 관리한다. `.claude/commands/*.md` 는 Claude slash command wrapper, `.agents/commands/*.md` 는 Codex command wrapper 로 둔다.
- Codex 는 Claude Code 의 slash command 시스템을 직접 실행하지 않는다. 사용자가 `/create-pr`, `/create-issue`, `/review-pr` 처럼 요청하면 `.agents/commands/` 의 wrapper 를 통해 공통 command 본문을 따른다.
...
- PR 형식·생성: 공통 command 절차 사용. Claude Code는 `.claude/commands/create-pr.md`, Codex는 `.agents/commands/create-pr.md` 를 따른다.
- 이슈 생성: 공통 command 절차 사용. Claude Code는 `.claude/commands/create-issue.md`, Codex는 `.agents/commands/create-issue.md` 를 따른다.

## 코드 배치 컨벤션

이 프로젝트의 결정 사항. AI agent 가 임의로 바꾸면 안 됨.
...
- 프로덕션 코드 변경이 기준을 초과해야 한다면 작업 전에 사용자에게 보고하고 승인 받기. 테스트·문서·기계적 rename은 필요한 범위에서 예외 가능.

...
| 판정 | 조건 | 처리 |
|---|---|---|
| **데이터** | 위키 정본 ≠ 시트 적재값 | 위키 `tools/*_drift_check.py` 실행 (**`sheets/` 최신 스냅샷으로 즉시 — 새 export 요구하지 않음**, 스냅샷 날짜 고지. 스냅샷 없거나 시트가 최근 수정됐을 때만 export 요청) → diff 리포트까지 여기서. **시트 수정은 위키 `grad-sheet-change` 로 인계** (정본 수정 → matrix 재생성 → 검증 → PR → apply CI. 사람 게이트 = PR 머지·운영 sync. 시트 직접 편집은 여전히 금지) |
...
3. 시트(라이브 구글시트)를 직접 수정 — 사람이든 에이전트든. 유일한 경로는 위키 `grad-sheet-change`

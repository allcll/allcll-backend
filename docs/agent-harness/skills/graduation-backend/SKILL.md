# 졸업요건 백엔드 코드 다리 (graduation-backend)
...
| 균형교양 | `docs/agent-harness/knowledge/graduation/conventions/loading-rules.md` §균형 + `BalanceRequired*` 엔티티 |
| 인증제 | `docs/agent-harness/knowledge/graduation/contracts/certification-policy.md` + `*CertCriterion` 엔티티 |
| 학번 보정·전공 예외 | `edge-cases.md` |

```
Knowledge 점검:
- 대상: docs/agent-harness/knowledge/graduation/conventions/resolver-layering.md, loading-rules.md
- 코드 일치: OK / 불일치(항목)
- 누락: 없음 / 있음
```

...
public class ChangedSubjectBuffer {
    ...
    public List<ChangeSubjectsResponse> getAllAndFlush() {
        ...
        int cnt = 0;
        while (cnt < sjptProperties.getRequestPerSecondCount() && (item = changedSubjectQueue.poll()) != null) {
            response.add(item);
            cnt++;
        }
        return response;

...
public class GraduationCertFetcher {
    ...
    private boolean fetchEnglish(OkHttpClient client) { // 영어 인증
        try {
            ...
            throw new AllcllException(AllcllErrorCode.ENGLISH_INFO_FETCH_FAIL);
        }
    }
    ...
    private boolean fetchCoding(OkHttpClient client) {
        try {
            ...
            throw new AllcllException(AllcllErrorCode.CODING_INFO_FETCH_FAIL);
        }
    }
    ...
    private boolean fetchClassicsPass(OkHttpClient client) { // 고전독서 인증
        try {
            ...
            try (Response response = client.newCall(request).execute()) {
                ...
                String certStatus = document.select(
                    ".b-con-box:has(h4.b-h4-tit01:contains(사용자 정보)) " +
                        "table tbody tr:has(th:contains(인증여부)) td"
                ).text().trim();

                // non pass 여부 검사
                if (certStatus.equals("아니오")) {
                    return false;
                }
                return true;
            }
        } catch (IOException exception) {
            throw new AllcllException(AllcllErrorCode.CLASSIC_INFO_FETCH_FAIL);
        }
    }
